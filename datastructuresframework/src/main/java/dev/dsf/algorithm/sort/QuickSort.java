...
public abstract class QuickSort {
    ...
    private static <T> int findPartitionIndex(T[] array, int minIndex, int maxIndex) {
        ...
        while (leftIndex <= rightIndex) {
            while (leftIndex <= rightIndex && (array[leftIndex] == null || (partitionElement != null
                    && ((Comparable<T>) array[leftIndex]).compareTo(partitionElement) < 0)))
                leftIndex++;
            while (rightIndex >= leftIndex && (array[rightIndex] != null && (partitionElement == null
                    || ((Comparable<T>) array[rightIndex]).compareTo(partitionElement) > 0)))
                rightIndex--;
